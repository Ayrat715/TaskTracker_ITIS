    ...
    <div class="register-container">
        <div class="register-form">
            ...
            <form @submit.prevent="register" class="form">
                ...
                <p class="login-link">Уже есть аккаунт?
...
export default {
    ...
    methods: {
        ...
        validatePassword(password) {
            ...
            const emailUser = this.email.split('@')[0].toLowerCase();

            if (password.length < 8) errors.push('Минимум 8 символов');

            if (/^\d+$/.test(password)) errors.push('Не только цифры');

            if (nameLower.length >= 3 && passLower.includes(nameLower)) {
                errors.push('Не используйте имя в пароле');
            }

            if (emailUser.length >= 3 && passLower.includes(emailUser)) {
                errors.push('Не используйте часть email в пароле');
            }

            const weakPasswords = ['password', '12345678', 'qwertyui', nameLower, emailUser];
            if (weakPasswords.includes(passLower)) {
